...
def detect(opt):
    ...
    all_pngs = glob.glob(source+'*atlas.png')
    set_logging()
    ...
    t0 = time.time()
    saved_dir = '/hpc/home/qh36/research/qh36/all_data/all_atlases/labeled_atlases/'
    all_circles = []
    all_labels = []
    for im in all_pngs:
        img = cv2.imread(im)
        ...
        if os.path.exists(corresponding_mrc):
            with mrcfile.open(corresponding_mrc) as mrc:
                dat = mrc.data
            # print(dat.shape)
            dat = cv2.normalize(dat, None, 0, 255,cv2.NORM_MINMAX)
            dat_multi_channel = np.dstack((dat, dat, dat))
            r, c = dat.shape 
            max_side = np.max([r,c])
            ...
            # cv2.imwrite(saved_dir+name+'.png',dat_multi_channel)
                    ...
                    im_re = cv2.resize(padded, (imgsz,imgsz))
                    ...
                    for i, det in enumerate(pred):  # detections per image
                        ...
                        gn = torch.tensor(padded.shape)[[1, 0, 1, 0]]  # normalization gain whwh
                        if len(det):
                            # Rescale boxes from img_size to im0 size
                            det[:, :4] = scale_coords(img_s.shape[2:], det[:, :4], padded.shape).round()
                            ...
                            for *xyxy, conf, cls in reversed(det):  # detections per image
                                ...
                                l,t,r,b = convert_yolo_to_cv(x,y,w,h, padded.shape[0], padded.shape[1])
                                ...
                                if if_sq:
                                    ...
                                    all_coords.append([l_m, t_m, r_m, b_m])
            ...
            if all_features.shape[0]>= 3:
                ...
                for idd, lbb in enumerate(labels):
                    ...
                    cv2.rectangle(dat_multi_channel, (l_m, t_m),(r_m, b_m),color_dict[lbb],1)
            elif all_features.shape[0] < 3 and all_features.shape[0]>0:
                for coords in all_coords:
                    l_m, t_m, r_m, b_m = coords[0],coords[1],coords[2],coords[3]
                    cv2.rectangle(dat_multi_channel, (l_m,t_m),(r_m,b_m),color_dict[0],1)
            ...
            # 
                # ind += 1
            cv2.imwrite(saved_dir+name+'_hand_atlas'+'.png', dat_multi_channel)
